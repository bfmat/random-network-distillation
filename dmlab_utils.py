...
# From https://raw.githubusercontent.com/google-research/episodic-curiosity/master/episodic_curiosity/environments/dmlab_utils.py
...
class DMLabWrapper(gym.Env):
  ...
  def __init__(self, platform, args,
    ...
    self._env = deepmind_lab.Lab(level_name, observation_format, string_args, renderer)
...
def make_dmlab(env_id, ep_path):

  env_settings = create_env_settings(env_id, seed=np.random.randint(1000000))
  ...
  env = CollectGymDataset(env, os.path.expanduser(ep_path), atari=False)
